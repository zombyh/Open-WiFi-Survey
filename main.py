...
class WifiPlannerApp:
    ...
    def save_image(self):
        ...
        for ap in self.model.aps:
            ...
            draw.text((ap.x + 8, ap.y - 12), ap.label, fill="#ff4a4a")
    ...
    def _draw_image_legend(self, draw, img_w, img_h):
        bx = img_w - 200
        by = img_h - 160
        draw.rectangle([bx-8, by-8, img_w-4, img_h-4],
                        fill=(20,20,32,210), outline=(90,90,120,255))
        ...
        draw.text((bx, by-4), "Sinal", fill=(160,170,220))
        for j, (c, lbl) in enumerate(items_sig):
            yy = by + 14 + j*20
            draw.rectangle([bx, yy, bx+16, yy+12], fill=c+(220,))
            draw.text((bx+22, yy), lbl, fill=(200,200,220))

        # Paredes
        draw.text((bx, by + 14 + 4*20 + 4), "Paredes", fill=(160,170,220))
        for j, (mat, cfg_w) in enumerate(WALL_TYPES.items()):
            yy = by + 14 + 4*20 + 20 + j*18
            hx = cfg_w["color"].lstrip("#")
            r, g, b = int(hx[0:2],16), int(hx[2:4],16), int(hx[4:6],16)
            draw.rectangle([bx, yy, bx+16, yy+10], fill=(r,g,b,220))
            draw.text((bx+22, yy), f"{mat} −{cfg_w['loss_db']}dB", fill=(200,200,220))
